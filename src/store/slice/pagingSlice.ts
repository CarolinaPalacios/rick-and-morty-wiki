...
import { getPageNumber } from '../../utils/paging'
...
export const pagingSlice = createSlice({
  ...
  reducers: {
    setPagingInfo: (state, action: PayloadAction<Info>) => {
      const nextPage =
        action.payload.next !== null ? getPageNumber(action.payload.next) : null
      const prevPage =
        action.payload.prev !== null ? getPageNumber(action.payload.prev) : null
